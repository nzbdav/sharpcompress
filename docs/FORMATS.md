...
4. The 7Zip format doesn't allow for reading as a forward-only stream, so 7Zip read support is only through the Archive API. Writing is supported through SevenZipWriter with LZMA/LZMA2 (independent or solid folders) and requires a seekable output stream. `SevenZipArchive` also implements the writable Archive API (`AddEntry`/`RemoveEntry`/`SaveTo`): there is no in-place editing, so `SaveTo` re-compresses the surviving source entries plus any pending additions through SevenZipWriter. Writing encrypted (AES) 7z output is not supported. See [7Zip Format Notes](#7zip-format-notes) for solid-folder reuse and async extraction details.
